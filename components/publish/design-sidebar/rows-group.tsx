 ...
 *
 * **The follow-up run is gated on the tile above it**, which is the rule the
 * rest of the designer follows: a pin size with no pin is a live-looking
 * control that changes nothing on the map under it.
 *
 * **"Link style" and "Link corners" used to sit below that**, four treatments
 * and four corners for the Directions and phone links. Both are gone: those
 * links are plain text, bold under the pointer, and a treatment nobody can
 * choose needs no control. See `packages/shared/snapshot.ts`, where the two
 * settings are retired, and `embed/src/styles.css` for the CSS. `rowActions`
 * stays — whether the links are drawn at all is still a real question.
 */
